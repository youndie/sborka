...
B-13 compares two test runs in-process because none of the four services has a JVM binary to start.
razves does: it is a CLI with `jvm()` and native executables on both `linuxX64` and `macosArm64`,
and "run it on a fixture, capture stdout, diff" needs no stand, no database and no service
lifecycle.

That makes it the portfolio's one instance of the process-level design, and worth having for a
reason beyond razves: where the two gates disagree about the same commit, the difference is exactly
what the in-process one cannot see.
...
- AC: a `parityCheck` on razves that runs both binaries over the fixture and fails on an unlisted
  difference in their output; whatever it finds that B-13's in-process gate does not, written into
