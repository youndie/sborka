...
if [ -d "$ROOT/sysroot" ]; then
    ...
    if build musl -PmuslSysRoot="$ROOT/sysroot" -PmuslLibGcc=../gcc -Pgc=noop; then
        ...
        # THROUGH `bash -c`, and a subshell will not do. The binary dies of SIGSEGV, and the shell
        # that waits on it announces that on ITS OWN stderr — so the report otherwise carries
        # "Segmentation fault" on a line that reads like a fault in this script. `( … ) 2>/dev/null`
        # does not help: the announcement comes from the shell OUTSIDE the parentheses. Only a shell
        # whose own stderr is redirected stays quiet, which is what this is. Checked both ways.
        bash -c "timeout -s KILL 10 strace -f -o /tmp/probe-noop.strace '$b' > /dev/null 2>&1" 2>/dev/null
        printf '  %-22s %s\n' "musl, noop, strace" \
