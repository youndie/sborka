...
ключом (`kompot.version`, `viddik.version`, `petich.version`), ключ переезжает в `version`, а шаг CI
«Determine version» — на composite action, который читает `version` по умолчанию.
...
Ещё лучше — прогнать `youndie/proba` по координатам в CI. Из одиннадцати публикующих репозиториев его
используют два.
